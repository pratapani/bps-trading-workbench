...
const loadLatestResults=(scanConfig=null)=>{
  fetch(`/latest_bps_results.csv?ts=${Date.now()}`)
    .then(r=>r.text())
    .then(t=>Papa.parse(t,{
      header:true,
      skipEmptyLines:true,
...
<div className="table"><table className={view==="all"?"completeTable":""}><thead><tr>{(view==="all"?["RK","STOCK","EXPIRY","SPOT","SELL","BUY","SELL BID","SELL OFFER","BUY BID","BUY OFFER","OTM%","OTM PTS","WIDTH","CREDIT","LOT","PROFIT/LOT","LOSS/LOT","BREAKEVEN","P:L","GRADE","SCORE"]:["RANK","STOCK","GRADE","SPOT","SELL","BUY","OTM","WIDTH","CREDIT","LOT","PROFIT/LOT","LOSS/LOT","BREAKEVEN","P:L"]).map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{paged.map((r,i)=><tr className={"clickable scoreRow "+gradeClass(r.GRADE)} onClick={()=>openStrategy(r)} key={r.STOCK+"-"+r.SELL+"-"+r.BUY+"-"+i}>{view==="all"?<><td>{r.RK}</td><td className="stock">{r.STOCK}</td><td>{r.EXPIRY||"—"}</td><td>{money(r.SPOT)}</td><td>{integer(r.SELL)}</td><td>{integer(r.BUY)}</td><td>{money(r.SELL_PE_BID)}</td><td>{money(r.SELL_PE_OFFER)}</td><td>{money(r.BUY_PE_BID)}</td><td>{money(r.BUY_PE_OFFER)}</td><td>{r["OTM%"].toFixed(2)}%</td><td>{r["OTM PTS"].toFixed(2)}</td><td>{integer(r.WIDTH)}</td><td>{money(r.CREDIT)}</td><td>{integer(r.LOT)}</td><td className="profit">{money(r["PROFIT/LOT"])}</td><td className="loss">{money(r["LOSS/LOT"])}</td><td>{money(r.BREAKEVEN)}</td><td><b>1:{r["P:L"].toFixed(2)}</b></td><td><Grade g={r.GRADE}/></td><td><b>{r.SCORE}</b></td></>:<><td><span className="rankBadge">{i+1}</span></td><td className="stock">{r.STOCK}</td><td><Grade g={r.GRADE}/></td><td>{money(r.SPOT)}</td><td>{integer(r.SELL)}</td><td>{integer(r.BUY)}</td><td>{r["OTM%"].toFixed(2)}%</td><td>{integer(r.WIDTH)}</td><td>{money(r.CREDIT)}</td><td>{integer(r.LOT)}</td><td className="profit">{money(r["PROFIT/LOT"])}</td><td className="loss">{money(r["LOSS/LOT"])}</td><td>{money(r.BREAKEVEN)}</td><td><b>1:{r["P:L"].toFixed(2)}</b></td></>}</tr>)}</tbody></table></div>
