...
const openStrategy=r=>{setSelected(r);setScenario({lots:1,spot:r.SPOT,sellPrice:r.SELL_PE_BID||0,buyPrice:r.BUY_PE_OFFER||0})};
const updateScenario=(key,value)=>setScenario(s=>({...s,[key]:value}));
const calc=useMemo(()=>{if(!selected||!scenario)return null;const lots=Math.max(1,num(scenario.lots));const spot=num(scenario.spot);const sellPrice=num(scenario.sellPrice);const buyPrice=num(scenario.buyPrice);const credit=sellPrice-buyPrice;const width=num(selected.SELL)-num(selected.BUY);const maxProfit=credit*num(selected.LOT)*lots;const maxLoss=(width-credit)*num(selected.LOT)*lots;const breakeven=num(selected.SELL)-credit;const otm=spot>0?((spot-num(selected.SELL))/spot)*100:0;const pl=maxProfit>0?maxLoss/maxProfit:0;return{lots,spot,sellPrice,buyPrice,credit,width,maxProfit,maxLoss,breakeven,otm,pl,quantity:num(selected.LOT)*lots};},[selected,scenario]);
...
<div className="table"><table><thead><tr>{["RANK","STOCK","GRADE","SCORE","SELL","BUY","OTM","WIDTH","CREDIT","LOT","PROFIT/LOT","LOSS/LOT","BREAKEVEN","P:L"].map(h=><th key={h}>{h}</th>)}</tr></thead>
<tbody>{paged.map((r,i)=><tr className={"clickable scoreRow "+gradeClass(r.GRADE)} onClick={()=>openStrategy(r)} key={r.STOCK+"-"+r.SELL+"-"+r.BUY+"-"+i}>
<td><span className={view==="best"?"rankBadge":""}>{view==="best"?((safePage-1)*pageSize+i+1):r.RK}</span></td><td className="stock">{r.STOCK}</td><td><Grade g={r.GRADE}/></td><td><b>{r.SCORE}</b></td>
<td>{integer(r.SELL)}</td><td>{integer(r.BUY)}</td><td>{r["OTM%"].toFixed(2)}%</td><td>{integer(r.WIDTH)}</td><td>{money(r.CREDIT)}</td><td>{integer(r.LOT)}</td><td className="profit">{money(r["PROFIT/LOT"])}</td><td className="loss">{money(r["LOSS/LOT"])}</td><td>{money(r.BREAKEVEN)}</td><td><b>1:{r["P:L"].toFixed(2)}</b></td>
</tr>)}</tbody></table></div>
...
<Chart title="Profit per lot" sub="Best qualified setups · green = higher profit"><ResponsiveContainer><BarChart data={best} margin={{top:10,right:15,left:10,bottom:5}}><CartesianGrid strokeDasharray="3 3" stroke="#dbe4ef"/><XAxis dataKey="STOCK"/><YAxis tickFormatter={integer}/><Tooltip contentStyle={{borderRadius:10,border:"1px solid #dbe4ef"}} formatter={v=>money(v)}/><Bar dataKey="PROFIT/LOT" name="Profit / lot" radius={[7,7,2,2]}>{best.map((r,i)=><Cell key={i} fill={r.GRADE==="A+"?"#16a34a":r.GRADE==="A"?"#0f766e":r.GRADE==="B"?"#2563eb":r.GRADE==="C"?"#f59e0b":"#ef4444"}/>)}</Bar></BarChart></ResponsiveContainer></Chart>
...
<div className="scenarioSection"><div className="sectionTitle">EDIT SCENARIO <span>changes recalculate instantly</span></div>
<div className="scenarioInputs"><label>Qty / Lots<input type="number" min="1" step="1" value={scenario.lots} onChange={e=>updateScenario("lots",Math.max(1,num(e.target.value)))}/><small>Total quantity: {integer(calc.quantity)}</small></label>
<label>Spot<input type="number" step=".05" value={scenario.spot} onChange={e=>updateScenario("spot",e.target.value)}/><small>Original: {money(selected.SPOT)}</small></label>
<label>Sell PE price<input type="number" step=".01" min="0" value={scenario.sellPrice} onChange={e=>updateScenario("sellPrice",e.target.value)}/><small>Scanner BID: {money(selected.SELL_PE_BID)} · OFFER: {money(selected.SELL_PE_OFFER)}</small></label>
<label>Buy PE price<input type="number" step=".01" min="0" value={scenario.buyPrice} onChange={e=>updateScenario("buyPrice",e.target.value)}/><small>Scanner BID: {money(selected.BUY_PE_BID)} · OFFER: {money(selected.BUY_PE_OFFER)}</small></label></div></div>
<div className="legs"><D k="SELL PUT" v={integer(selected.SELL)}/><D k="BUY PUT" v={integer(selected.BUY)}/><D k="WIDTH" v={money(calc.width)}/></div>
...
<div className="detailGrid"><D k="NET CREDIT" v={money(calc.credit)}/><D k="BREAKEVEN" v={money(calc.breakeven)}/><D k="OTM" v={calc.otm.toFixed(2)+"%"}/><D k="DISTANCE TO BE" v={calc.spot>0?(((calc.spot-calc.breakeven)/calc.spot)*100).toFixed(2)+"%":"—"}/><D k="P:L" v={calc.pl>=0?"1:"+calc.pl.toFixed(2):"Invalid"}/><D k="LOT SIZE" v={integer(selected.LOT)}/></div>
<div className="executionNote"><b>Execution inputs</b><span>SELL BID → {money(selected.SELL_PE_BID)} · BUY OFFER → {money(selected.BUY_PE_OFFER)}</span><em>Editable prices above are scenario values; the CSV remains unchanged.</em></div>
<div className="why"><b>Scenario analysis</b><p>Strike and lot size come from the selected scanner row. Spot and option prices are editable so you can test different fills, market levels and position sizes without changing the source CSV.</p></div>
</aside></div>}
